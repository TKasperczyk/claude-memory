...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-surface-2 p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-1 focus:ring-ring/50 disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="h-4 w-4" />
